...
alter table public.ashara_equipment add column if not exists category text not null default '';
...
create index if not exists ashara_equipment_workstream_idx on public.ashara_equipment (workstream);
create index if not exists ashara_equipment_category_idx on public.ashara_equipment (category);
...
drop policy if exists "Internal read equipment photos" on storage.objects;
drop policy if exists "Internal write equipment photos" on storage.objects;
create policy "Internal read equipment photos" on storage.objects for select using (bucket_id = 'equipment-photos');
create policy "Internal write equipment photos" on storage.objects for all using (bucket_id = 'equipment-photos') with check (bucket_id = 'equipment-photos');
